...
use crate::structs::shared_buffer::internal::vtable::{PROMO64_EVEN_VT, PROMO_EVEN_VT};
...
pub (crate) fn promo_is_unique<T>(h: &AtomicPtr<()>) -> bool {
   ...
   if raw.is_null() {
       ...
       unsafe { (*(raw as *const PromotableVec<T>)).ref_cnt.load(Ordering::Acquire) == 1 }
   }
}
...
pub (crate) unsafe fn promo_clone(
   h: &AtomicPtr<()>,
   ptr: *const u8,
   len: usize,
) -> SharedBuffer {
...
pub (crate) unsafe fn promo_drop(
   h: &mut AtomicPtr<()>,
   _p: *const u8,
   _l: usize,
) {
...
pub (crate) unsafe fn promo64_clone(
   h: &AtomicPtr<()>,
   ptr: *const u8,
   len: usize,
) -> SharedBuffer {
...
/// Decrements reference count, deallocating if last reference.
pub (crate) unsafe fn promo64_drop(
   h: &mut AtomicPtr<()>,
   _p: *const u8,
   _l: usize,
) {
